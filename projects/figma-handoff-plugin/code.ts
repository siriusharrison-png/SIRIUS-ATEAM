...
figma.ui.onmessage = async (msg) => {
  if (msg.type === "generate-code") {
    ...
    const selection = figma.currentPage.selection;
    if (selection.length > 0) {
      await addSpecsToFigma(selection[0], msg.markdown, msg.componentName);
    }
  }
};
...
// 提取节点数据
function extractNodeData(node: SceneNode): object {
  const data: any = {
    id: node.id,
    name: node.name,
    type: node.type,
  ...
  // 子节点
  if ("children" in node) {
    data.children = (node as FrameNode).children.map(extractNodeData);
  }

  return data;
}

function extractFill(fill: Paint): object {
  if (fill.type === "SOLID") {
    return {
      type: "SOLID",
      color: {
...
// 保存规范到节点
async function saveSpecsToNode(node: SceneNode, specs: object) {
  node.setPluginData("handoff-specs", JSON.stringify(specs));
}

// 从节点加载规范
async function loadSpecsFromNode(node: SceneNode): Promise<object | null> {
...
// 将规范添加到 Figma 画布
async function addSpecsToFigma(node: SceneNode, markdown: string, componentName: string) {
  try {
    // 加载字体
    ...
    if ("x" in node && "y" in node && "width" in node) {
      frame.x = node.x + (node as FrameNode).width + 40;
      frame.y = node.y;
    }
...
// 颜色映射到 Tailwind 类名
function colorToTailwind(color: { r: number; g: number; b: number }, type: string): string {
...
// 提取 Tailwind 类名
function extractClassNames(node: any): string[] {
...
// 提取文字类名
function extractTextClasses(node: any): string[] {
  const classes: string[] = [];
...
// 生成子节点代码
function generateChildren(node: any, indent: number): string {
...
// 生成组件代码
function generateCode(nodeData: any, specs?: any): string {
